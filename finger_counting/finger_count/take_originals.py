...
def generate_augmented_images():
    ...
    for file in path:
        ...
        flip = [0]
        rotate = random.sample(range(-20, 20), 3)
        shiftx = random.sample(range(-10, 10), 2)
        shifty = random.sample(range(-10, 10), 2)
        zoom = random.sample(range(0, 20), 3)
...
def binaryMask(img):
    ...
    img = cv2.adaptiveThreshold(img, maxValue=255, adaptiveMethod=cv2.ADAPTIVE_THRESH_GAUSSIAN_C, thresholdType=cv2.THRESH_BINARY_INV, blockSize=9, C=2)
    ret, img = cv2.threshold(img, thresh=20, maxval=255, type=cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    return img
